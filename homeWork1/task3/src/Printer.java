...
### Задача №3 ###
-------------------------------------------------------------------------------
Реализуйте метод divArrays, принимающий в качестве аргументов два целочисленных
массива a и b, и возвращающий новый массив с, каждый элемент которого равен
...
class Answer {
    public int[] divArrays(int[] a, int[] b) {
        int[] divVarietyArr = new int[a.length];
        if (a.length != b.length) return new int[1];
        for (int i = 0; i < a.length; i++) {
            int valueA = a[i];
            int valueB = b[i];
            int divVariety = this.getDivVariety(valueA, valueB);
            divVarietyArr[i] = divVariety;
        }
        return divVarietyArr;
    }

    public int getDivVariety(int number1, int number2) {
